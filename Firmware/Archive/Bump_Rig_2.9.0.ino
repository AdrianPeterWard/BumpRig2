...
float x_total_wheel_travel = 0.0;
int n_Laps = 0;
...
int cycleCount = 0;
int minSensorValue = analogRead(3);  // Replace A0 with the actual pin you're using
int maxSensorValue = minSensorValue;
unsigned long cycleStartTime = 0;
unsigned long cycleDuration = 500; // Assuming a frequency of 2Hz, so one cycle lasts for 500ms
...
eui_message_t tracked_variables[] =
    {
        ...
        EUI_FLOAT("CalMax", V_Stroke_Sensor_Cal_max),
        EUI_FLOAT("CalMin", V_Stroke_Sensor_Cal_min),
        ...
        EUI_INT16("x_travel", x_total_wheel_travel),
        EUI_INT16("Laps", n_Laps),
...

{
  ...
  V_Stroke_Sensor_Cal_min = (analogRead(potPin1) / 1023.0) * 3.3; // V 

  delay(1000);

  digitalWrite(relayPin, LOW); // retract

  V_Stroke_Sensor_Cal_max = (analogRead(potPin1) / 1023.0) * 3.3; // V
...
void loop()
{
  ...
  float x_delta_position = abs(position - previous_position);
...
// Detect the end of a cycle
  if (millis() - cycleStartTime >= cycleDuration) {
    // Calculate the travel distance for this cycle
    float cycleDistance = maxSensorValue - minSensorValue;
    totalCycleDistance += cycleDistance;
    ...
    // Reset the cycle start time for the next cycle
    cycleStartTime = millis();
  }

averageCycleDistance = totalCycleDistance / cycleCount;
