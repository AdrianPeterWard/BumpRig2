...
const errorDataSource = new MessageDataSource('n_error')
const previouserrorDataSource = new MessageDataSource('n_previous_error')
...
function FilePicker(props: {
  onSelected: (filepath: string | null) => void
  ...
  const pathPicker = useCallback(() => {
    ...
    ipcRenderer
      .invoke(OPEN_DIALOG_IPC_EVENT, options)
...
function DataViewer(props: {
  ...
  const [remainingRepeats, setRemainingRepeats] = useState(10)
  ...
  useEffect(() => {
    ...
    // Send message
    const message = new Message('set_point', row.setPoint)
    ...
    const cancellationToken = new CancellationToken(
      `set point iteration ${activeIndex}`,
    ).deadline(1000)
  ...
  const currentOverallIndex =
    activeIndex +
    (totalLoopIterations - remainingRepeats - 1) * props.data.length
  ...
  const treeData: TreeNodeInfo[] = props.data.map((row, index) => ({
    id: index,
    label: `Set Point: ${row.setPoint}mm`,
    icon: (
      <Icon
        icon={IconNames.DOT}
        intent={
        ...
        {isRunning ? (
          ...
          <>
            <Button
              ...
              style={{ width: '25%' }}
              disabled={
                activeIndex === props.data.length - 1 && remainingRepeats === 0
              }
            >
              Start
            </Button>
...
function DataInjestor() {
  ...
  const parsedData = useMemo(() => {
    ...
    try {
      ...
      for (let index = 0; index < parsed.length; index++) {
        ...
        throw new Error(
          `Error reading CSV at index ${index}, row: ${JSON.stringify(row)}`,
        )
      }
      ...
      if (err.code === 'ENOENT') {
        ...
        setErrorMessage(
          `An unknown error occurred while reading the file: ${err.message}`,
        )
      }
  ...
  return (
    <>
      <FilePicker
        onSelected={setSelectedFilePath}
      ...
      {parsedData ? (
        <DataViewer data={parsedData} setErrorMessage={setErrorMessage} />
      ) : null}
    </>
  )
}
